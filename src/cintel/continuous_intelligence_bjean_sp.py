...
- The CSV file includes these columns:
  ...
  - date of service: date and time service occurred
  - service code: all data is 615d or direct community support
...
Paths (relative to repo root)
    ...
    OUTPUT FILE: artifacts/system_assessment_bjean.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.continuous_intelligence_bjean_sp
...
DATA_FILE: Final[Path] = DATA_DIR / "ServiceProvisionData.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "system_assessment_service_provision.csv"
...
MAX_Duration: Final[float] = 60.0  # 60 minutes threshold
...
def main() -> None:
    ...
    df = df.with_columns(
        (pl.col("duration").mean()).round(0).cast(pl.Int64).alias("avg_duration")
    )
    ...
    anomalies_df = df.filter(pl.col("avg_duration") > MAX_Duration)
    LOG.info(f"STEP 3. Using thresholds: MAX_Duration={MAX_Duration}")
    ...
    summary_df = df.select(
        [
            pl.col("avg_duration").mean().alias("avg_duration"),
        ]
    )

    # Add a simple assessment label
    summary_df = summary_df.with_columns(
        pl.when(pl.col("avg_duration") > MAX_Duration)
        .then(pl.lit("ANOMALY"))
        .otherwise(pl.lit("STANDARD"))
        .alias("duration_state")
    )
